...
class AwsDeployFunction {
  ...
  checkIfFunctionChangesBetweenImageAndHandler() {
    ...
    if (functionObject.image && remoteFunctionPackageType === 'Zip') {
      const errorMessage = [
        `The function "${this.options.function}" you want to update with image was previously packaged as zip file.`,
        ' Please run "serverless deploy" to ensure consistent deploy.',
      ];
      throw new this.serverless.classes.Error(
        errorMessage,
  ...
  async callUpdateFunctionConfiguration(params) {
    // TODO: ADD max count?
    const callWithRetryUntilSuccess = async () => {
      try {
        ...
        if (err.providerError && err.providerError.code === 'ResourceConflictException') {
          ...
          await wait(5000);
          await callWithRetryUntilSuccess();
        } else {
          throw err;
        }
      }
